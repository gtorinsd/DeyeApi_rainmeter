...
class Worker:
    ...
    def _get_device_data_list_param(data_list, param_names: List) -> Dict:
        ...
        return {x: {'value': dict_values[x]['value'], 'value_str': f"{dict_values[x]['value']} {dict_values[x]['unit']}"} for x in param_names}
    ...
    def work(self, station='2508271645') -> Optional[Dict]:
        if self.api_client.auth():
            ...
            result.update(self._get_device_data_list_param(data_list=device_info, param_names=['TotalGridPower', 'BatteryVoltage']))
